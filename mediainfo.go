...
func (file *File) Get(streamKind StreamKind, streamNumber int, parameter string, infoKind InfoKind) string {
	...
	return C.GoString(C.GetMediaInfo(file.handle, C.MediaInfo_stream_C(streamKind), C.size_t(streamNumber), cParameter, C.MediaInfo_info_C(infoKind), C.MediaInfo_info_C(0)))
}
...
func Open(filePath string) (*File, error) {
	...
	handle := C.OpenMediaFile(cFilePath)
	if handle == nil {
